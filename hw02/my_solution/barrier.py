...
if __name__ == "__main__":
    ...
    while 1:
        ...
        while 1:
            ...
            fprimeprime = np.zeros_like(a*a.T)###YOUR CODE HERE### 

            #compute the first and second derivatives from each hyperplane and aggregate
            for j in range(0,numplanes):
                fprime_for_plane_j = -1*a[:,j]/(b[j]- np.transpose(a[:,j])*x) ###YOUR CODE HERE###

                fprimeprime_for_plane_j = -1*fprime_for_plane_j*np.transpose(a[:,j])/(b[j]- np.transpose(a[:,j])*x)###YOUR CODE HERE###
            ...
            step = -1*np.linalg.inv(fprimeprime)*fprime###YOUR CODE HERE###

            #compute the Newton decrement squared (in terms of step and fprimeprime)
            lambda2 =  np.transpose(fprime)*np.linalg.inv(fprimeprime)*fprime###YOUR CODE HERE###

            #check if we've reached the Newton's method stopping condition
            #if so, break out of Newton's method
            if(lambda2/2<=epsilon):###YOUR CODE HERE###):
                break;
            ...
            while 1:
                ...
                if(pastboundary or fnew>f+alpha*k*np.transpose(fprime)*step):###YOUR CODE HERE###):  #put in the check for terminating backtracking line search
                    #if we're not done
                    k = beta*k###YOUR CODE HERE###
                else:
                    break;
